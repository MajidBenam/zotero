...
with open(goodname, "r") as jsonfile:
    ...
    for ref in a_dictionary:
        ...
        helping_dic["zotero_id"] = ref["id"].split("/")[-1]
        ...
        if "issued" in ref.keys():
            ...
            # handling bad years
            helping_dic["year"] = None
            #print(ref["id"])
        # main author (editor) family name:
        if "author" in ref.keys():
            if "family" in ref["author"][0].keys():
                helping_dic["creator"] = ref["author"][0]["family"]
            elif "literal" in ref["author"][0].keys():
                helping_dic["creator"] = ref["author"][0]["literal"].split(" ")[-1]
        elif "editor" in ref.keys():
            if "family" in ref["editor"][0].keys():
                helping_dic["creator"] = ref["editor"][0]["family"]
            elif "literal" in ref["editor"][0].keys():
                helping_dic["creator"] = ref["editor"][0]["literal"].split(" ")[-1]
        else:
            helping_dic["creator"] = None
            print(ref['id'])
        # what we want to do is to write to a json file, with keys being our search terms:
        mother_helping_list.append(helping_dic)
        # write to a csv
        #csv_writer.writerow([helping_dic["creator"], helping_dic["title"], helping_dic["year"], "No Description", date.today(), date.today(), helping_dic["URL"], helping_dic["zotero_id"]])

with open('shorter_JSON_GSed.json', 'w') as json_file_s_s:
    json.dump(mother_helping_list, json_file_s_s, ensure_ascii=False)
    
print(len(mother_helping_list))